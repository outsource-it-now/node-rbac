...
export default class DynamooseMTStorage extends Storage {
  constructor(options = {}) {
    super();
    ...
    if (!options.tenant) {
      throw new Error('Parameter tenant is undefined.');
    }
  ...
  add(item, cb) {
    ...
    if (!this.isPermission(item)) {
      newInstance.tenant = this._options.tenant;
    }
  ...
  remove(item, cb) {
    ...
    let scanParams = { grants: { contains: name } };

    if (!this.isPermission(item)) {
      scanParams = { grants: { contains: [name, this._options.tenant] } };
    }

    this.model.scan(scanParams, (err, records) => {
      if (err) {
        return cb(err);
      }
  ...
  grant(role, child, cb) {
    ...
    this.model.queryOne({ name: { eq: name }, type: { eq: Type.ROLE }, tenant: { eq: this._options.tenant } }, (err, record) => {
      if (err) {
        return cb(err);
      }

      if (!record.grants) {
        this.model.update({ name: name, type: Type.ROLE, tenant: this._options.tenant }, { grants: [childName] }, (err) => {
          if (err) {
            return cb(err);
          }
  ...
  get(name, cb) {
    ...
    this.model.queryOne({ name: { eq: name }, tenant: { eq: this._options.tenant } }, (err, record) => {
      if (err) {
        return cb(err);
      }
  ...
  getRoles(cb) {
    ...
    this.model.query({ type: { eq: Type.ROLE }, tenant: { eq: this._options.tenant } }, (err, records) => {
      if (err) {
        return cb(err);
      }
  ...
  isPermission(item) {
    if (getType(item) === Type.PERMISSION) {
      return true;
    }
